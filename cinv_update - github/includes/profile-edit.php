<?php
	...
	//validate idGroup
	$idGroup = ['Student','Teacher'];
	if(empty($_POST['idGroup']))
	{
		$info['errors']['idGroup'] = "An Identify is required";
	}else
	if(!in_array($_POST['idGroup'], $idGroup))
	{
		$info['errors']['idGroup'] = "idGroup is not valid";
	}
	...
	if(empty($info['errors']) && $row)
	{
		...
		db_query("update users set firstname = :firstname,lastname = :lastname,idGroup = :idGroup,date = :date,email = :email $image_query $password_query where id = :id limit 1",$arr);
